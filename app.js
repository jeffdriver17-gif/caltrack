...
function ProfileScreen({
  ...
  async function fetchProfiles() {
    ...
    const {
      data,
      error
    } = await supabase.from('ct_profiles').select('*').order('created_at', {
      ascending: false
    });
    if (!error && data) {
      setProfiles(data);
  ...
  async function createProfile() {
    ...
    const {
      data,
      error
    } = await supabase.from('ct_profiles').insert([p]).select().single();
    if (error) {
  ...
  async function attemptLogin() {
    ...
    const {
      data,
      error
    } = await supabase.from('ct_profiles').select('*').eq('id', loginModal.id).eq('pin_hash', hashedPin).single();
    setLoading(false);
    if (error || !data) {
      setLoginModal({
        ...loginModal,
...
function App({
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const [logRes, recRes, goalRes, waterRes] = await Promise.all([supabase.from('ct_food_logs').select('*').eq('profile_id', profileId), supabase.from('ct_recipes').select('*').eq('profile_id', profileId), supabase.from('ct_goals').select('*').eq('profile_id', profileId).single(), supabase.from('ct_water').select('*').eq('profile_id', profileId)]);
      if (logRes.data) {
  ...
  async function updateGoals(newGoals) {
    setGoals(newGoals);
    await supabase.from('ct_goals').upsert({
  ...
  async function updateWater(dateStr, glasses) {
    ...
    await supabase.from('ct_water').upsert({
      profile_id: profileId,
      date: dateStr,
      glasses
    });
  ...
  async function addEntry(food, qty, meal) {
    ...
    await supabase.from('ct_food_logs').insert([entry]);
  }
  async function removeEntry(id) {
    ...
    await supabase.from('ct_food_logs').delete().eq('id', id);
  }
  async function saveRecipe(recipe) {
    ...
    if (isUpdate) {
      ...
      await supabase.from('ct_recipes').upsert(data);
    } else {
      setRecipes(prev => [...prev, data]);
      showToast('Recipe saved!');
      await supabase.from('ct_recipes').insert([data]);
    }
  }
  async function deleteRecipe(id) {
    setRecipes(prev => prev.filter(r => r.id !== id));
    showToast('Recipe deleted');
    await supabase.from('ct_recipes').delete().eq('id', id);
  }
